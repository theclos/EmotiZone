...
public class GeminiResp {
    ...
    public  static void getResponse(ChatFutures chatModel, String query, ResponseCallback callback){
        Content.Builder userMessgeBuilder = new Content.Builder();
        userMessgeBuilder.setRole("user");
        userMessgeBuilder.addText(query);
        Content userMessage = userMessgeBuilder.build();
    ...
    public GenerativeModelFutures getModel(Context context){
        // Asegúrate de que BuildConfig está inicializado
        if (BuildConfig.apiKey == null) {
            BuildConfig.initialize(context);
        }
